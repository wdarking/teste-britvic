...
<div class="row justify-content-center">
    <div class="col-md-12">
        ...
        <div class="card">
            <div class="card-header">{{ __('Edit Reservation: :description', ['description' => $reservation->description]) }}</div>
                ...
                <form action="{{ route('reservations.update', [$reservation]) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 @error('vehicle_id') is-invalid @enderror" name="vehicle_id" id="reservationVehicle">
                            ...
                            <option @if($vehicle->id == $reservation->vehicle->id) selected="selected" @endif value="{{ $vehicle->id }}">{{ $vehicle->name }} | {{ $vehicle->year }} |
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 @error('user_id') is-invalid @enderror" name="user_id" id="reservationUser">
                            ...
                            @foreach($users as $user)
                                <option @if($user->id == $reservation->user->id) selected="selected" @endif value="{{ $user->id }}">{{ $user->name }}</option>
                            @endforeach
                        </select>
                        @error('user_id')
                        <span class="invalid-feedback" role="alert">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control datetimepicker @error('date') is-invalid @enderror"
                            id="reservationDate" name="date" value="{{ old('date') ?? $reservation->date }}">
                        @error('date')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
